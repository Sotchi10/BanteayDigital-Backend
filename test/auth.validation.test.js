...
test('login accepts exactly one contact identifier', () => {
  assert.equal(loginSchema.safeParse({ email: 'sokha@gmail.com', password: 'password123' }).success, true)
...
test('auth validation requires an accepted email domain and a 9 to 10 digit local phone number', () => {
  ...
  assert.equal(registerSchema.safeParse({ email: 'person@example.com', password: 'password123' }).success, false)
  assert.equal(registerSchema.safeParse({ email: 'person@student.cadt.edu.kh', password: 'password123' }).success, true)
...
test('profile updates accept only safe editable account fields', () => {
  assert.equal(updateProfileSchema.safeParse({ name: 'Sreynich Chan', username: 'sreynich_1', email: 'sreynich@gmail.com' }).success, true)
